...
describe("Abc Testing", () => {
  let owner: SignerWithAddress;
  let signers: SignerWithAddress[];
  let token: Token1;
  let staking: AbcStaking;

  beforeEach(async () => {
    signers = await ethers.getSigners();
    owner = signers[0];
    token = await new Token1__factory(owner).deploy("Yato", "YTC", 1000000);
    staking = await new AbcStaking__factory(owner).deploy(token.address);
    await token
      .connect(owner)
      .transfer(staking.address, expandTo18Decimals(10000));
    await token
      .connect(owner)
  ...
  describe("AbcTest", async () => {
    ...
    it.only("Claimable Reward", async () => {
      ...
      console.log(
        "Claimable Rewards 1:   " +
          (await staking.connect(signers[1]).claimableReward(1))
      );
      console.log(
        "Claimable Rewards 2:   " +
          (await staking.connect(signers[1]).claimableReward(2))
      );
    });
    it("Claim Rewards before and after stake time", async () => {
      ...
      await network.provider.send("evm_mine");
      console.log(
        "Old balance in tokens:  " + (await token.balanceOf(signers[1].address))
      );
      await staking.connect(signers[1]).claimReward(1);
      console.log(
        "New balance in tokens:  " + (await token.balanceOf(signers[1].address))
      );
      console.log(
        "Transaction history:  " +
          (await staking.connect(signers[1]).checkTransaction(1))
      );
    });
